...
BEGIN
	...
	ELSE
		...
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'S00000000' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
...
(
	...
	TrangThai NVARCHAR(40)
		CHECK (TrangThai IN (N'',N'Đang kinh doanh', N'Đang chờ xét duyệt thêm', N'Đang chờ xét duyệt tạm ngừng kinh doanh', N'Đang chờ xét duyệt cập nhật thông tin', N'Tạm ngừng kinh doanh', N'Lịch sử cập nhật')),
